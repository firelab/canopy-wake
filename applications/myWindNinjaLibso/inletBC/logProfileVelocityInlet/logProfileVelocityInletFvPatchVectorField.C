...
void Foam::logProfileVelocityInletFvPatchVectorField::setTerrainHeights()
{
	        ...
	        if(mag(currentPt.z()-faceCenterPoints[pointJ].z()) > 0.1 )
 	        {
		        ...
		        if(curPatchName == "xMin" || curPatchName == "xMax")
		        {
		            checkOrdinate_FC = faceCenterPoints[pointJ].y();
		            checkOrdinate_CP = currentPt.y();
		        }
		        else if(curPatchName == "yMax" || curPatchName == "yMin")
		        {
		        ...
		        // Look for cells close to x or y coord (depending on the face) of currentPt    // The bracket for checking the ordinates has been set to 1
		        if( mag(checkOrdinate_FC - checkOrdinate_CP) < 100 )                            // if( mag(checkOrdinate_FC - checkOrdinate_CP) < 10 )
	            {
    ...
    {
        Info << "In logProfileVelocityInlet::setTerrainHeights(). Found relativeHeights below z0, correcting by adding z0 to all relativeHeights" << endl;
...
//- Construct by mapping given logProfileVelocityInletFvPatchVectorField
//  onto a new patch
Foam::logProfileVelocityInletFvPatchVectorField::
logProfileVelocityInletFvPatchVectorField
(
...
void Foam::logProfileVelocityInletFvPatchVectorField::updateCoeffs()
{
    if (updated())
    {
        return;
...
//- Write
void Foam::logProfileVelocityInletFvPatchVectorField::write(Ostream& os) const
{
