...
template<class CloudType>
void Foam::myConeInjection<CloudType>::setThetaDistributionType()
{
    ...
    if (thetaDistType == "original" || thetaDistType == word::null)     // sets this as the default if not found in the dictionary
    {
        thetaDistributionType_ = original;
    }
    else if (thetaDistType == "uniform")
    {
    ...
    else if (thetaDistType == "normal")
    {
        ...
        this->coeffDict().lookup("theta_variance") >> theta_variance;
    }
    else
    {
        FatalErrorInFunction
            << "thetaDistributionType must be either 'original', "
            << "'uniform' or 'normal'"
            << exit(FatalError);
    }
}
...
:
    ...
    theta_variance(10)  // this default value may or may not be the best value
{
...
:
    ...
    Cd_(im.Cd_),
    Pinj_(im.Pinj_),
    theta_variance(im.theta_variance)
{}
...
)
{
    ...
    switch (injectionMethod_)
    {
        case imPoint:
        {
            if ( thetaDistributionType_ == original || thetaDistributionType_ == uniform )
            {
            ...
            {
                const scalar beta = twoPi*rndGen.scalar01();
                const scalar frac = rndGen.scalarNormal();
                const vector n = normalised(direction_.value(t));
                const vector t1 = normalised(perpendicular(n));
                const vector t2 = normalised(n ^ t1);
                tanVec = t1*cos(beta) + t2*sin(beta);
            ...
            switch (thetaDistributionType_)
            {
                ...
                case normal:
                {
                    ...
                    theta =
                        degToRad
                        (
                            theta_variance*rndGen.scalarNormal()
                        );
                    break;
                }
                default:
